...
// Serve the current trail
app.get('/playbackTrailPoints.json', (req, res) => {
  fs.readFile('playbackTrailPoints.json', 'utf8', (err, data) => {
    if (err) return res.status(404).send('[]');
    res.type('application/json').send(data);
  });
});

// Receive new trail from the simulation
app.post('/uploadTrail', (req, res) => {
  fs.writeFile('playbackTrailPoints.json', JSON.stringify(req.body, null, 2), err => {
    if (err) return res.status(500).send('fail');
    res.send('ok');
  });
});

app.listen(PORT, () => console.log(`Trail server running at http://localhost:${PORT}`));
...
const PLAYBACK_INTERVAL_MS = 300; // ms per playback step (~50 FPS)

/* Helper: center the mapContainer so the point (x,y) in container coords is centered in the viewport */
function centerOnPoint(x, y, smooth = true) {
  const container = document.getElementById('mapContainer');
  if (!container) return;
  // ensure transform origin is top-left
  container.style.transformOrigin = '0 0';
  // compute translation so point (x,y) is at (viewportWidth/2, viewportHeight/2)
...
function playbackStep(timestamp){
  ...
  // after setting followerEl.style.left/top inside playbackStep:
if (typeof centerOnPoint === 'function') {
  centerOnPoint(pos.left, pos.top, true);
}
  ...
  // inside playbackStep loop:
const pos = playbackTrailPoints[playbackIdx];
if(pos && followerEl){
  followerEl.style.left = pos.left + 'px';
  followerEl.style.top  = pos.top  + 'px';

  // center viewport on follower
  if (typeof centerOnPoint === 'function') {
    centerOnPoint(pos.left, pos.top, true);
  }
}
...
// Place this after buildPlaybackTrailFromRecorded(), for example in a debug function
function downloadPlaybackTrail(){
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(playbackTrailPoints));
    const dlAnchor = document.createElement('a');
    dlAnchor.setAttribute("href", dataStr);
    dlAnchor.setAttribute("download", "playbackTrailPoints.json");
    document.body.appendChild(dlAnchor); // required for firefox
    dlAnchor.click();
    dlAnchor.remove();
}
// Call downloadPlaybackTrail() from browser console after sim run.
// --- BEGIN: Trail uploader for Android live sync ---
function sendTrailToServer() {
  const url = "http://localhost:8080/uploadTrail"; // or your LAN IP when on real device

  fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(playbackTrailPoints)
  })
  .then(resp => {
    if(resp.ok) {
      alert("Live path uploaded!");
    } else {
      alert("Error uploading path: " + resp.status);
    }
  })
  .catch(e => alert("Error: " + e));
}

// To test: open console and run sendTrailToServer() after a simulation.
